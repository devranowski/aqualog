    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid w-full items-center gap-4">
        <div className="flex flex-col space-y-2">
          ...
          <DateInput date={date} setDate={setDate} isMobile={isMobile || false} />
        </div>
      ...
      <div className="flex justify-end">
        <Button type="submit" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? 'Adding value...' : 'Add value'}
        </Button>
      </div>
    </form>
  );
...
export const ParameterCard = React.memo(
  function ParameterCard({
    ...
    const { addLog, isSubmitting: localIsSubmitting } = useParameterLogs({
      aquariumId,
      parameters
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      const success = await addLog(title, newValue, date);
      if (success) {
        setNewValue('');
        setDate(undefined);
    ...
    const formContent = React.useMemo(
      ...
      [title, date, newValue, handleValueChange, handleSubmit, isMobile, isSubmitting]
    );
